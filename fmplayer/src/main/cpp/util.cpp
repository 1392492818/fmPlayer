 ...
 * @return
 */
long long bytes2long(byte *b)
{
    long long s = 0;
    long long s0 = b[0] & 0xff;// 最低位
    long long s1 = b[1] & 0xff;
    long long s2 = b[2] & 0xff;
    long long s3 = b[3] & 0xff;
    long long s4 = b[4] & 0xff;// 最低位
    long long s5 = b[5] & 0xff;
    long long s6 = b[6] & 0xff;
    long long s7 = b[7] & 0xff;
...
bool h264_decode_sps(BYTE * buf,unsigned int nLen,int &width,int &height,int &fps)
{
    ...
    if(nal_unit_type==7)
    {
        ...
        if(vui_parameter_present_flag)
        {
            ...
            LOGE("timing_info_present_flag %d ", timing_info_present_flag);
